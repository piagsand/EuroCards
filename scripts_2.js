...
function displayGroups() {
    ...
    topNavigation.id = 'top-navigation';  // Ajout d'un ID pour la navigation
    container.innerHTML = ''; // remise à zéro 
    ...
    Object.keys(euroCards).forEach(group => {
        ...
        groupHeader.onclick = () => {  // Ajout du gestionnaire de clics
...
//Fonctions qui s'exécute au chargement
onload = function () {
    displayGroups();

    updateList('missingCards', missingCards);
    updateList('ownedCards', ownedCards);
    updateList('duplicateCards', duplicateCards, true);
};
    /* ******************************************************* */ 
...
//     cardInput.value = '';  // Nettoyer l'input après extraction
...
//         if (!cardNumber) return;  // Ignorer les entrées vides
//         if (validCards.has(cardNumber)) {  // Vérifier si la carte est valide
//             if (ownedCards.includes(cardNumber)) {
//                 duplicateCards.push(cardNumber);  // Gérer les doublons
//             } else {
//                 ownedCards.push(cardNumber);  // Ajouter aux cartes possédées
//                 addedCards.push(cardNumber);
//             }
//         } else {
//             invalidCards.push(cardNumber);  // Gérer les cartes invalides
//         }
//     });

//     updateList('ownedCards', ownedCards.sort());  // Mettre à jour la liste affichée

//     // Afficher les messages appropriés
//     displayMessage(addedCards, duplicateCards, invalidCards);
// }
...
//     cardNumbers.forEach(cardNumber => {
//         if (!cardNumber) return;  // Ignorer les entrées vides
//         if (isCardValid(cardNumber)) {  // Vérifier si la carte est valide
...
//                 addedCards.push(cardNumber);
//                 if (index !== -1) {
//                     missingCards.splice(index, 1);  // Retirer de la liste des manquantes
//                 }
//             }
//         } else {
//             invalidCards.push(cardNumber);  // Gérer les cartes invalides
...
//     updateList('ownedCards', ownedCards);
//     updateList('missingCards', missingCards);  // Mise à jour de la liste des cartes manquantes
    ...
    cardNumbers.forEach(cardNumber => {
        ...
        if (isCardValid(cardNumber)) {  
            ...
            if (ownedCards.includes(cardNumber)) {
                ...
                ownedCards.push(cardNumber);
                addedCards.push(cardNumber);
                // Retirer de la liste des manquantes
                if (index !== -1) {
...
function moveSelectedToOwned() {
    ...
    checkboxes.forEach(checkbox => {
        ...
        // Optionnellement, supprimer l'élément de la liste visuelle
    ...
    function updateList(listId, items) {
        ...
        if (Array.isArray(items)) {
            // Si c'est un tableau, trier et afficher chaque élément
            ...
            // Si c'est un objet, trier par clé et afficher clé + comptage
            Object.keys(items).sort().forEach(key => {
                const listItem = document.createElement('li');
                listItem.textContent = `${key} (${items[key]}x)`;
                list.appendChild(listItem);
            });
        } else {
            console.error('updateList was passed an unsupported type of items:', items);
        }
    }
...
function createListItem(cardNumber, duplicateCount = null) {
    ...
    // Texte à afficher avec la carte
 
    // Ajouter un écouteur de clic sur l'élément `li` pour cocher/décocher
...
function markAsDuplicate() {
    ...
    checkboxes.forEach(checkbox => {
        const cardNumber = checkbox.value;
        duplicateCards[cardNumber] = (duplicateCards[cardNumber] || 0) + 1;
        checkbox.checked = false; // Désélectionner la checkbox après traitement
    });
    updateList('duplicateCards', duplicateCards, true);
}
    /* ******************************************************* */ 
...
function markGivenCards() {
    ...
    givenInput.value = ''; // Effacer le champ après l'opération
...
function markCheckedAsGiven() {
    ...
    checkboxes.forEach(checkbox => {
        const cardNumber = parseInt(checkbox.value);
        if (duplicateCards[cardNumber] > 0) {
            duplicateCards[cardNumber] -= 1;
            if (duplicateCards[cardNumber] === 0) {
                delete duplicateCards[cardNumber];
